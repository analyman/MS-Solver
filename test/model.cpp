...
void TESTB() //{
{
    ...
    auto n5 = x.new_node(0.0 * BUnit.m, 1.0 * BUnit.m);
    ...
    auto b1 = x.new_Beam(n1, n5, Connector::FixedConnect, Connector::FixedConnect, 10.0 * EB, 1.0 * AB, 10.0 * IB);
    auto bx = x.new_Beam(n5, n2, Connector::FixedConnect, Connector::FixedConnect, 10.0 * EB, 1.0 * AB, 10.0 * IB);
...
//    x.new_Beam(n4, n2, Connector::FixedConnect, Connector::FixedConnect, 10000.0 * EB, 100.0 * AB, 100.0 * IB);
    ...
    auto l2 = BasicLoad<double>(false, 400.0 * BUnit.KN / BUnit.m, 0.0);

    x.getBeam(b1)->setLoad(l2);
    x.getBeam(bx)->setLoad(l2);
...
void TESTC() //{
{
    SMSolverManager<double> x;
    auto n1 = x.new_node(0.0 * BUnit.m, 0.0 * BUnit.m);
    auto n2 = x.new_node(1.0 * BUnit.m, 0.0 * BUnit.m);
    auto n3 = x.new_node(2.0 * BUnit.m, 0.0 * BUnit.m);
...
//    x.new_Beam(n4, n2, Connector::FixedConnect, Connector::FixedConnect, 10000.0 * EB, 100.0 * AB, 100.0 * IB);
    ...
    auto l1 = BasicLoad<double>(false, 5.0 * BUnit.KN / BUnit.m, 0.0);
    auto l2 = BasicLoad<double>(false, 10.0 * BUnit.KN / BUnit.m, 0.0);
...
void TESTD() //{
{
    ...
    auto b1 = x.new_Beam(n1, n3, Connector::FixedConnect, Connector::FixedConnect, 10.0 * EB, 1.0 * AB, 10.0 * IB);
