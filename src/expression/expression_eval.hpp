...
class CircularList //{
{
    ...
    public:
        ...
        value_type pop_front()
        {
            ...
            delete (this->m_data + this->m_head);
        ...
        value_type pop_back()
        {
            ...
            delete (m_data + (m_head + m_size - 1) % m_capacity);
            m_size--;
            return x;
        }
...
accept_token_type nextAcceptance(token_enum t) //{
{
    switch(t){
        ...
        case token_enum::ImmediateValue:
            return accept_token_type::op_abc;
        case token_enum::Id:
            return accept_token_type::op_abcd;
...
#define EVAL_BINARY_OPERATOR  \
    ...
    m_val_stack.pop(); \
    if(!(first->GetToken().GetLevel() == op.GetLevel() && second->GetToken().GetLevel() == op.GetLevel())) return; \
    \
...
{
    assert(!(m_val_stack.empty() || m_operator_stack.empty()));
    Token& op = m_operator_stack.top();
    ...
    if(op.GetType() != token_enum::Function){ // operator
        assert(tp->GetToken().GetLevel() == op.GetLevel() + 1);
...
handle_func:
    ...
    public:
        ...
        APT<Token>* GetParseTree() //{
        {
            ...
            if(m_operator_stack.empty() && m_val_stack.size() == 1)
